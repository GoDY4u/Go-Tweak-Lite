...
function Install-VisualsAndDirectX {
    Write-Log "Redirecting to official Microsoft downloads..."
    
    try {
        # Visual C++ Redistributable (latest version)
        Write-Log "Opening Visual C++ Redistributable download page..."
        Start-Process "https://aka.ms/vs/16/release/vc_redist.x64.exe"
        
        # DirectX End-User Runtime
        Write-Log "Opening DirectX End-User Runtime download page..."
        Start-Process "https://www.microsoft.com/en-us/download/details.aspx?id=35"
        
        Write-Log "Please download and install from official Microsoft links" "SUCCESS"
        Write-Log "Run the installers as Administrator for best results" "INFO"
        return $true
    } catch {
        Write-Log "Error opening browser: $($_.Exception.Message)" "ERROR"
        Write-Log "Please manually visit: https://aka.ms/vs/16/release/vc_redist.x64.exe" "WARNING"
        return $false
    }
}
        ...
        do {
            ...
            switch ($choice) {
                ...
                "7" { 
                    ...
                    if ($result) {
                        Write-Host "✅ Redirected to official Microsoft downloads" -ForegroundColor Green
                    } else {
