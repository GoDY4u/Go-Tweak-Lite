...
function Show-EnhancedMenu {
    ...
    Write-Host "┌──────────────────────────────────────────────────┐" -ForegroundColor DarkGray
    Write-Host "│ 17. 🔑 Activate Windows (MAS)                   │" -ForegroundColor Yellow
    Write-Host "│ 18. 🛒 Install Microsoft Store (LTSC)           │" -ForegroundColor Yellow
    Write-Host "│ 19. 🧹 Chris Titus Debloat Tool                 │" -ForegroundColor Yellow
    Write-Host "│ 20. 🧹 Raphi Debloat Script                     │" -ForegroundColor Yellow
    Write-Host "└──────────────────────────────────────────────────┘" -ForegroundColor DarkGray
    
    Write-Host ""
    Write-Host "┌──────────────────────────────────────────────────┐" -ForegroundColor DarkGray
    Write-Host "│ 21. 🚪 Exit Program                            │" -ForegroundColor Red
    Write-Host "└──────────────────────────────────────────────────┘" -ForegroundColor DarkGray
    Write-Host ""
}
...
# Function to activate Windows using MAS
function Invoke-WindowsActivation {
    Write-Log "Starting Windows activation with MAS..."
    Show-LoadingAnimation "Launching activation script..." 2
    
    try {
        Write-Host "`nExecuting: irm https://get.activated.win | iex" -ForegroundColor Cyan
        $scriptBlock = { irm https://get.activated.win | iex }
        & $scriptBlock
        Write-Log "Windows activation script executed" "SUCCESS"
        Show-Notification "Windows Activation" "Activation script has been launched. Follow the instructions." "Success"
        return $true
    } catch {
        Write-Log "Error running activation: $($_.Exception.Message)" "ERROR"
        Show-Notification "Activation Error" "Could not execute activation script" "Error"
        return $false
    }
}

# Function to install Microsoft Store on Windows LTSC
function Install-MicrosoftStoreLTSC {
    Write-Log "Starting Microsoft Store installation for LTSC..."
    Show-LoadingAnimation "Installing Microsoft Store..." 2
    
    try {
        Write-Host "`nRunning: wsreset -i" -ForegroundColor Cyan
        Start-Process -FilePath "wsreset.exe" -ArgumentList "-i" -NoNewWindow -Wait
        Write-Log "Microsoft Store installation attempted" "SUCCESS"
        
        Show-Notification "Microsoft Store" "Installation process started. Check the Store app in your Start Menu." "Success"
        return $true
    } catch {
        Write-Log "Error installing Microsoft Store: $($_.Exception.Message)" "ERROR"
        Show-Notification "Store Installation Error" "Could not install Microsoft Store. Try running the command manually: wsreset -i" "Error"
        return $false
    }
}

# Function to launch Chris Titus debloat tool
function Invoke-ChristitusDebloat {
    Write-Log "Starting Chris Titus debloat tool..."
    Show-LoadingAnimation "Launching debloat tool..." 2
    
    try {
        Write-Host "`nExecuting: iwr -useb https://christitus.com/win | iex" -ForegroundColor Cyan
        $scriptBlock = { iwr -useb https://christitus.com/win | iex }
        & $scriptBlock
        Write-Log "Chris Titus debloat tool executed" "SUCCESS"
        Show-Notification "Debloat Tool" "The debloat tool is now running. Follow the on-screen prompts." "Success"
        return $true
    } catch {
        Write-Log "Error running debloat tool: $($_.Exception.Message)" "ERROR"
        Show-Notification "Debloat Error" "Could not execute debloat tool" "Error"
        return $false
    }
}

# Function to launch Raphi debloat script
function Invoke-RaphiDebloat {
    Write-Log "Starting Raphi debloat script..."
    Show-LoadingAnimation "Launching Raphi debloat script..." 2
    
    try {
        Write-Host "`nExecuting: & ([scriptblock]::Create((irm 'https://debloat.raphi.re/')))" -ForegroundColor Cyan
        $scriptBlock = { & ([scriptblock]::Create((irm "https://debloat.raphi.re/"))) }
        & $scriptBlock
        Write-Log "Raphi debloat script executed" "SUCCESS"
        Show-Notification "Raphi Debloat" "The debloat script is now running. Follow the on-screen prompts." "Success"
        return $true
    } catch {
        Write-Log "Error running Raphi debloat script: $($_.Exception.Message)" "ERROR"
        Show-Notification "Debloat Error" "Could not execute Raphi debloat script" "Error"
        return $false
    }
}
...
function Main {
    ...
    if ($GUI) {
        ...
        do {
            Show-EnhancedMenu
            $choice = Read-Host "Select an option (1-21)"
            ...
            switch ($choice) {
                ...
                "17" { 
                    Write-Log "Activating Windows with MAS..."
                    $result = Invoke-WindowsActivation
                    Read-Host "Press Enter to continue..."
                }
                "18" { 
                    Write-Log "Installing Microsoft Store on LTSC..."
                    $result = Install-MicrosoftStoreLTSC
                    Read-Host "Press Enter to continue..."
                }
                "19" { 
                    Write-Log "Launching Chris Titus Debloat Tool..."
                    $result = Invoke-ChristitusDebloat
                    Read-Host "Press Enter to continue..."
                }
                "20" { 
                    Write-Log "Launching Raphi Debloat Script..."
                    $result = Invoke-RaphiDebloat
                    Read-Host "Press Enter to continue..."
                }
                "21" { 
                ...
                default {
                    Show-Notification "Invalid Option" "Please select a valid option (1-21)" "Error"
                    Start-Sleep -Seconds 1
                }
            }
        } while ($choice -ne "21")
