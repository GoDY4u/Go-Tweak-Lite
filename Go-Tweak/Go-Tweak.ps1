...
$script:VisualsPath = Join-Path $ContentPath "visualsc and directx"
...
function Initialize-ProjectStructure {
    ...
    $essentialFolders = @($ScriptsPath, $RAMPath, $InternetPath, $GlobalOptimizationPath, $VisualsPath)
...
# Function to install Visual C++ and DirectX components
function Install-VisualsAndDirectX {
    Write-Log "Starting Visual C++ and DirectX installation"
    
    # Install Visual C++ Runtimes
    $visualCBat = Join-Path $VisualsPath "visualc\visual-c-runtimes-all-in-one-jul-2025\install_all.bat"
    if (Test-Path $visualCBat) {
        Write-Log "Installing Visual C++ Runtimes..."
        Start-Process -FilePath $visualCBat -Wait -Verb RunAs
        Write-Log "Visual C++ Runtimes installed successfully" "SUCCESS"
    } else {
        Write-Log "Visual C++ Runtimes installer not found" "WARNING"
    }
    
    # Install DirectX
    $directXBat = Join-Path $VisualsPath "directx\directx-redist-jun-2010\install_all.bat"
    if (Test-Path $directXBat) {
        Write-Log "Installing DirectX..."
        Start-Process -FilePath $directXBat -Wait -Verb RunAs
        Write-Log "DirectX installed successfully" "SUCCESS"
    } else {
        Write-Log "DirectX installer not found" "WARNING"
    }
    
    Write-Log "Component installation completed" "SUCCESS"
}
        ...
        do {
            ...
            switch ($choice) {
                ...
                "7" { 
                    ...
                    Install-VisualsAndDirectX
                    Write-Host "✅ Component installation completed" -ForegroundColor Green
                    Read-Host "Press Enter to continue..."
                }
                "8" { 
                    Write-Log "Opening keyboard properties..."
