# install.ps1 - INSTALLER CON EMOJIS SEGUROS
...
try {
    ...
    if ($extractedFolders.Count -gt 0) {
        ...
        Write-Host "[F] Moving files from: $($extractedFolders[0].Name)" -ForegroundColor Cyan
    ...
    if (-not (Test-Path $otherTweaksPath)) {
        Write-Host "[F] Creating othertweaks folder..." -ForegroundColor Cyan
    ...
    if (-not (Test-Path $msAppsPath)) {
        Write-Host "[F] Creating ms-apps folder..." -ForegroundColor Cyan
    ...
    Get-ChildItem -Path $installPath -Recurse -Directory | ForEach-Object {
        Write-Host "    [F] $($_.FullName.Replace($installPath, ''))" -ForegroundColor White
    }
    Get-ChildItem -Path $installPath -Recurse -File | ForEach-Object {
        Write-Host "    [F] $($_.FullName.Replace($installPath, ''))" -ForegroundColor Gray
    }
    ...
    if (Test-Path $mainScriptPath) {
        ...
        Write-Host "[X] Main script not found: Go-Tweak.ps1" -ForegroundColor Red
